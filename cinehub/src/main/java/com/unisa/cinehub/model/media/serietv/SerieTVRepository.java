...
public interface SerieTVRepository extends JpaRepository<SerieTv, Long> {
    ...
    @Query("select case when count(s) > 0 then true else false end from SerieTv s where lower(s.titolo) like lower(:title) and s.annoUscita = :annoUscita and s.visibile = true")
